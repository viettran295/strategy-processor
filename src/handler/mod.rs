...
use crate::{fetch::StockFetcher, scanner::{Backtest, ScannerPerformance, ScannerCrossingMA}};
...
enum ProcessStrategy {
    ResultStrategy(Box<dyn FnOnce(DfConverter, &Query<QueryParams>) -> Result<String, Box<dyn std::error::Error>>>),
    OptionStrategy(Box<dyn FnOnce(&Query<QueryParams>) -> Option<(&String, &f32)>>)
}
    ...
    match fetcher.fetch_prices(symbol.to_string(), start_date, end_date).await {
        Ok(stock_data) => {
            ...
            match df_proc.to_df(stock_data) {
                Ok(_) => {
                    match process_fn{
                        ProcessStrategy::ResultStrategy(resutl_fn) => {
                            match resutl_fn(df_proc, query) {
                                Ok(response) => {
                                    return HttpResponse::Ok()
                                        .content_type("application/json")
                                        .body(response);
                        ...
                        _ => {
                            return HttpResponse::InternalServerError()
                                .body(format!("Error processing data"))
                        }
                    }
                },
                Err(e) => {
...
async fn get_ma_signal(
    ...
    fetch_and_process(
        symbol, 
        &query, 
        ProcessStrategy::ResultStrategy(
            Box::new(|df_proc, query| {
